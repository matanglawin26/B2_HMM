...
class HiddenMarkovModel:
    ...
    def get_probability(self, observations):
        ...
        for chain in product(self.states, repeat=chain_length):
            ...
            p_observations = self.emission_matrix.df[chain[0]
                                                     ].df[observations[0]]
    ...
    def display_probabilities(self, observations_list):
        for observations in observations_list:
            print("Score for {} is {:f}.".format(
                observations, self.get_probability(observations)))

    def display_optimal_sequence(self, observations_list):
        for observations in observations_list:
            optimal_sequence, max_probability = self.get_optimal_hidden_sequence(
                observations)

            print("Given the known model and the observation {}, the weather was most likely {} with ~{:0.2f}% probability.".format(
                observations, optimal_sequence, max_probability * 100))
...
def main():
    ...
    transition_matrix = Matrix(
        {'Rainy': hidden_state1, 'Sunny': hidden_state2})

    emission_matrix = Matrix(
        {'Rainy': observable_state1, 'Sunny': observable_state2})
